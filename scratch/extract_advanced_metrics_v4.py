...
# Dynamic DummyClass to mock any PyTorch or Opacus class structure
...
class StubModule(types.ModuleType):
    def __getattr__(self, name):
        # Return DummyClass for any attributes/classes
        if name == 'device':
            return DummyDevice
        return DummyClass

# Install finder to mock torch and opacus imports
...
# Populate core sys.modules entries to prevent finder recursion
sys.modules['torch'] = StubModule('torch')
sys.modules['torch.utils'] = StubModule('torch.utils')
sys.modules['torch.utils.data'] = StubModule('torch.utils.data')
sys.modules['opacus'] = StubModule('opacus')
...
if os.path.exists(tabpfn_path):
    try:
        ...
        # Test if pred length is 10000 or 1000
