...
df_temp['selectivity_ratio'] = df_temp['mutual_matches'] / (df_temp['likes_received'] + 1)
...
# 
# > [!NOTE]
# > **Performance Optimization:** This causal modeling block runs 100 bootstrap iterations. It is protected by a high-speed `joblib` caching layer (`../models_v7/dml_causal.joblib`). Subsequent runs skip model fitting and bootstrap estimation entirely, loading the ATE coefficient, bootstrap standard errors, and p-values instantly in **0.01 seconds**.
################################################################################
...
cache_file = '../models_v7/dml_causal.joblib'
...
import os, joblib
os.makedirs('../models_v7', exist_ok=True)
cache_file_boruta = '../models_v7/boruta_support.joblib'
...
models = {
    ...
    'XGBoost': XGBClassifier(scale_pos_weight=1),  # Neutral: SMOTE already balances classes 50/50, n_estimators=500, random_state=RANDOM_STATE, eval_metric='logloss', **TREE_CONFIG['xgb'], n_jobs=-1),
...
# Set RETRAIN_BASELINE = False to load pre-trained results from models_v7 if available.
...
checkpoint_path = '../models_v7/baseline_results.joblib'
results = {}
temp_results = {}
...
# Check if we should load the full baseline results from models_v7
...
cv_checkpoint_path = '../models_v7/cv_results.joblib'
cv_results = {}
temp_cv = {}
...
lc_checkpoint_path = '../models_v7/learning_curve_data.joblib'
lc_data = {}
temp_lc = {}
...
os.makedirs('../models_v7', exist_ok=True)
cache_dp = '../models_v7/opacus.joblib'
...
os.makedirs('../models_v7', exist_ok=True)
cache_file = '../models_v7/gnn_gat.joblib'
...
import os, joblib
os.makedirs('../models_v7', exist_ok=True)
cache_scarf = '../models_v7/scarf.joblib'
...
os.makedirs('../models_v7', exist_ok=True)
cache_tabpfn = '../models_v7/tabpfn.joblib'
...
def run_optuna_search(X_tr, y_tr, X_te, y_te):
    ...
    import os, joblib
    os.makedirs('../models_v7', exist_ok=True)
    cache_optuna = '../models_v7/optuna_pareto.joblib'
...
# Set RETUNE_MODELS = False to load pre-tuned results from models_v7 if available.
RETUNE_MODELS = False           
# -----------------------------------------------------------------
...
checkpoint_tuned_path = '../models_v7/tuned_results.joblib'
tuned_results = {}
temp_tuned = {}
...
# Check if we should load the full tuned results from models_v7
        ...
        base_models = {
            ...
            'Champion Stacking Ensemble': StackingClassifier(
                estimators=[
                    ...
                    ('xgb', XGBClassifier(scale_pos_weight=1)),  # Neutral: SMOTE already balances classes 50/50, n_estimators=500, random_state=RANDOM_STATE, eval_metric='logloss', tree_method='hist', device=XGB_DEVICE, n_jobs=1) if HAS_XGBOOST else GradientBoostingClassifier(n_estimators=500, random_state=RANDOM_STATE)),
...
os.makedirs('../models_v7', exist_ok=True)
cache_pycaret = '../models_v7/pycaret_results.joblib'
...
try:
    if os.path.exists(cache_pycaret):
        ...
        clf1 = setup(data=train_df, target='target', session_id=123, verbose=False, html=False, use_gpu=False, n_jobs=-1)
        
        # Compare all standard models and return the best one
        print("Comparing models...")
        best_pycaret_model = compare_models(exclude=['svm']) # Exclude SVM as it hangs for 40+ mins on 50k rows # No budget, run exhaustively
...
# Ensure models_v7 exists
os.makedirs('../models_v7', exist_ok=True)
cache_hstat = '../models_v7/h_stat.joblib'
...
os.makedirs('../models_v7', exist_ok=True)
cache_shap = '../models_v7/shap_interactions.joblib'
...
import os, joblib
os.makedirs('../models_v7', exist_ok=True)
cache_mapie = '../models_v7/mapie.joblib'
...
# 2. Cache Routing
os.makedirs('../models_v7', exist_ok=True)
cache_bayesian = '../models_v7/bayesian_model.joblib'
...
if champion_name:
    target_dict[champion_name]['y_pred'] = calibrated_clf.predict(X_test)
    target_dict[champion_name]['y_prob'] = calibrated_clf.predict_proba(X_test)[:, 1]
    print(f"\n✅ Updated results['{champion_name}'] with calibrated predictions.")
    from sklearn.metrics import accuracy_score, f1_score, roc_auc_score
    cal_acc = accuracy_score(y_test, target_dict[champion_name]['y_pred'])
    cal_f1 = f1_score(y_test, target_dict[champion_name]['y_pred'])
    cal_auc = roc_auc_score(y_test, target_dict[champion_name]['y_prob'])
...
# 
# > [!NOTE]
# > **Performance Optimization:** Algorithmic recourse searches high-dimensional continuous and categorical feature spaces using randomized search, which takes substantial processing time. We wrapped this recourse search in a dynamic `joblib` cache (`../models_v7/dice_recourse.joblib`), which reloads and renders the diverse counterfactual recourse dataframes instantly on subsequent runs.
################################################################################
...
os.makedirs('../models_v7', exist_ok=True)
cache_file = '../models_v7/dice_recourse.joblib'
...
# 
# > [!NOTE]
# > **Performance Optimization:** Uplift modeling requires training separate treatment and control response estimators. We wrapped this meta-classifier in a high-speed `joblib` cache (`../models_v7/causal_uplift.joblib`), storing the estimators and individual treatment effect scores to render downstream segment charts instantly.
################################################################################
...
os.makedirs('../models_v7', exist_ok=True)
cache_file = '../models_v7/causal_uplift.joblib'
...
# * **Double-Path Routing:** Dedicated dual-path directory routing has been implemented, reading the computationally heavy pre-trained SVM from `../models/` while saving the new training runs dynamically to `../models_v7/`, ensuring 100% thread-safety and protecting original files.
# 
# ---
# 
...
# - **`models_v7/baseline_results.joblib`**: Stores all trained baseline models and prediction variables.
# - **`models_v7/cv_results.joblib`**: Stores all 5-fold cross-validation scores.
# - **`models_v7/learning_curve_data.joblib`**: Stores pre-computed learning curve coordinates.
# - **`models_v7/tuned_results.joblib`**: Stores all tuned estimators and grid search parameters.
# - **`models_v7/flaml_results.joblib`**: Stores the trained FLAML AutoML estimator.
# 
