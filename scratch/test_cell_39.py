...
models = {
    ...
    'FT-Transformer': PyTorchSklearnClassifier(model_class=FTTransformer, epochs=1, lr=0.005),
    'SAINT': PyTorchSklearnClassifier(model_class=SAINT, epochs=1, lr=0.005),
    'NODE': PyTorchSklearnClassifier(model_class=NODE, epochs=1, lr=0.005),
...
# Run fit/predict on a small dummy dataset for all models to verify!
X_dummy = np.random.randn(10, 5)
y_dummy = np.random.randint(0, 2, size=10)

for name, model in models.items():
    if name == 'SVM':
        # Skip SVM to save time during verification
        continue
    print(f"Fitting model {name}...")
    model.fit(X_dummy, y_dummy)
    preds = model.predict(X_dummy)
    probs = model.predict_proba(X_dummy)
    print(f"[OK] {name} works! preds shape: {preds.shape}, probs shape: {probs.shape}")
