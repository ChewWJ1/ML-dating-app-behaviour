...
print("Injecting ultimate SCARF optimizations: caching t-SNE coordinates and fast parallel downstream RF classifier...")
...
if os.path.exists(cache_scarf):
    ...
    # Load or dynamically pre-compute and update t-SNE cache
    if 'raw_2d' in scarf_data and 'embed_2d' in scarf_data:
        raw_2d = scarf_data['raw_2d']
        embed_2d = scarf_data['embed_2d']
    else:
    ...
    with torch.no_grad():
        ...
        # Calculate t-SNE coordinates for caching on first run
        from sklearn.manifold import TSNE
...
print("SUCCESS: SCARF cell updated with t-SNE coordinates cache and fast classifier!")
