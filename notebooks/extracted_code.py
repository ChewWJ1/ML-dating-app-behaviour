...
if os.path.exists(cache_file):
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(W)):
        ...
        model_T = RandomForestClassifier(n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1)
        ...
        model_Y = RandomForestClassifier(n_estimators=50, random_state=RANDOM_STATE, n_jobs=-1)
...
models = {
    ...
    'Decision Tree': DecisionTreeClassifier(class_weight='balanced', random_state=RANDOM_STATE),  # Depth-constrained
    'Random Forest': RandomForestClassifier(class_weight='balanced', n_estimators=500, max_features='sqrt', random_state=RANDOM_STATE, n_jobs=-1),  # Depth-constrained to prevent SMOTE noise memorisation
    ...
    'Balanced Random Forest': BalancedRandomForestClassifier(n_estimators=500, random_state=RANDOM_STATE, n_jobs=-1),  # Depth-constrained
    ...
    'SVM': BaggingClassifier(
        ...
        n_estimators=num_threads, max_samples=0.40, n_jobs=-1, random_state=RANDOM_STATE
    ),
}
...
X_tr_tensor = torch.tensor(X_train.values if hasattr(X_train, 'values') else X_train, dtype=torch.float32).to(DEVICE)
y_tr_tensor = torch.tensor(y_train.values if hasattr(y_train, 'values') else y_train, dtype=torch.float32).to(DEVICE)
...
model_att.eval()
X_te_tensor = torch.tensor(X_test.values if hasattr(X_test, 'values') else X_test, dtype=torch.float32).to(DEVICE)
with torch.no_grad():
...
clf_scarf = RandomForestClassifier(n_estimators=100, random_state=RANDOM_STATE, n_jobs=-1)
...
if 'results' in globals() and results:
    ...
    clf_raw = RandomForestClassifier(n_estimators=100, random_state=RANDOM_STATE, n_jobs=-1)
...
if y_pred_tab is None:
    ...
    sub_indices = np.random.choice(len(X_train_arr), size=min(3000, len(X_train_arr)), replace=False)
...
if not loaded_tuned_from_cache:
    ...
    for name in top3:
        ...
        base_models = {
            ...
            'Random Forest': RandomForestClassifier(class_weight='balanced', max_features='sqrt', random_state=RANDOM_STATE),
            ...
            'SVM': BaggingClassifier(
                ...
                n_estimators=32, max_samples=0.40, n_jobs=1, random_state=RANDOM_STATE
            ),
        }
...
try:
    ...
    if os.path.exists(flaml_checkpoint_path):
        ...
        automl.fit(
            ...
            task="classification",
            metric="roc_auc",
            time_budget=7200,  # 2 hours budget
            n_jobs=1,
            verbose=0,
            seed=RANDOM_STATE
        )
...
if best_tree_name:
    ...
    if tree_model:
        ...
        X_sample = X_test.iloc[:2000] if hasattr(X_test, 'iloc') else pd.DataFrame(X_test[:2000], columns=X.columns)
...
axes[1].barh(range(min(15, len(feature_names))), 
             np.abs(X_adv[:100] - X_test.values).mean(axis=0)[:15],
             color='#e17055')
